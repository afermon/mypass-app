...
public class UserService implements UserInterface {
    ...
    @Override
    public void update(final MyPassUser mypassUser, final OnUpdateUserListener listener) {
        JhiAccount account = new JhiAccount(true, mypassUser.getEmail(), mypassUser.getFirstName(), null, Locale.getDefault().getLanguage(), mypassUser.getLastName(), mypassUser.getLogin());
        ...
        call.enqueue(new Callback<Void>() {
            ...
            public void onResponse(Call<Void> call, Response<Void> response) {
                if (response.code() == 200) { // OK
                    synchronized (userLock) {
                        UserService.this.mypassUser = mypassUser;
                    }
                    listener.onUpdateUserSuccess(mypassUser);
    ...
    public void getLogedUser(OnUserAvailableListener listener) {
        synchronized (userLock) {
            if (mypassUser != null) {
                listener.onUserAvailable(mypassUser);
            } else {
                loadUser();
    ...
    private void onUserResponse(JhiAccount account) {
        synchronized (userLock) {
            gettingUser = false;
            mypassUser = new MyPassUser(account.getLogin(), account.getEmail(), account.getFirstName(), account.getLastName());
